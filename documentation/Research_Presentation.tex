...
\begin{frame}{Why Proportional Linear Allocation?}
\begin{center}
...
\small
Comparison of allocation framework performance across the full sample period. The proportional linear allocation method (Strategy 3) demonstrated superior risk-adjusted returns compared to alternative allocation frameworks tested, while maintaining implementable portfolio constraints and preserving the tactical signal logic from Strategy 2.
\end{frame}
...
\begin{frame}{Monthly Strategy Performance}
\begin{columns}[c]
\column{0.6\textwidth}
\begin{center}
...
\textbf{Monthly Strategy Metrics:}
...
\small
Both Strategy 3 and Monthly Strategy substantially outperform the static benchmark across the full sample period, with Monthly Strategy delivering consistent returns while respecting operational trading constraints.
\end{frame}
...
\textbf{Key Observation:} Much of Strategy 3's predictive power derives from risk reduction and the drawdown differential override mechanism rather than pure directional signals.
...
\begin{itemize}
    ...
    \item The framework successfully navigated major market stress events including the 2008 financial crisis, 2020 COVID-19 selloff, and 2022 interest rate shock
\end{itemize}
\end{frame}
