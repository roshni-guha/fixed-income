...
\begin{itemize}
    \item \textbf{H5A4 (High Yield)}: Represented by HYG, providing exposure to corporate high-yield bonds with higher credit risk and return potential
    \item \textbf{C5A0 (Investment Grade)}: Represented by LQD, offering investment-grade corporate bond exposure with moderate credit risk
    \item \textbf{G5O2 (Government Bonds)}: Represented by TLT, providing long-duration Treasury exposure as a defensive anchor
...
\begin{figure}[H]
    ...
    \caption{BMOM indicators for fixed income assets over time, showing the evolution of blended momentum signals across market cycles.}
    \label{fig:bmom_timeseries}
\end{figure}

\subsection{Strategy 1: Cross-Asset Momentum with Benchmark Filter}

Strategy 1 generates binary signals (long or flat) for each asset independently by combining asset-level BMOM with cross-asset benchmark filtering. This approach identifies periods when both an asset and its relevant benchmark exhibit weakening momentum, indicating broader market stress rather than asset-specific volatility.

\textbf{Benchmark Pairing}

Each fixed income asset is paired with a benchmark that captures its primary risk factor:
\begin{itemize}
    \item \textbf{H5A4 (High Yield)} $\rightarrow$ SPY: High yield bonds are sensitive to equity risk sentiment and economic growth expectations
    \item \textbf{C5A0 (Investment Grade)} $\rightarrow$ LQD: Investment grade credit reflects broader corporate credit conditions
    \item \textbf{G5O2 (Government)} $\rightarrow$ TLT: Long-duration Treasuries serve as their own benchmark for interest rate regime changes
\end{itemize}
...
The cross-below detection mechanism (rather than static threshold evaluation) identifies momentum deterioration in real time. Requiring dual weakness (both asset and benchmark below threshold) filters out idiosyncratic moves and confirms broader risk-off conditions. The uniform $-0.03$ threshold provides a consistent definition of momentum weakness across all assets, while the asset-specific benchmarks ensure that exit signals reflect relevant market dynamics for each credit and duration exposure.
...
\midrule
Annualized Return & 1.21\% & 0.74\% \\
Annualized Volatility & 2.68\% & 2.96\% \\
Sharpe Ratio & 0.45 & 0.25 \\
Max Drawdown & $-11.71\%$ & $-11.99\%$ \\
Win Rate & 42.57\% & 50.79\% \\
Rebalance Days & 587 & 325 \\
Trades per Month & 1.75 & 0.97 \\
\bottomrule
\end{tabular}
...
Strategy 3's daily monitoring with turnover filtering achieved superior risk-adjusted returns (Sharpe 0.45) by rebalancing approximately 1.75 times per month when momentum signals or constraint violations warranted adjustment. The Monthly Strategy, constrained by Regentfund's operational requirement of monthly execution, reduced trading frequency to 0.97 times per month, resulting in lower performance (Sharpe 0.25) but meeting institutional implementation requirements.

The risk-triggered rebalancing mechanism preserved some tactical responsiveness within the monthly constraint: the 0.97 trades per month (slightly below 1.0) reflects scheduled monthly rebalances plus occasional mid-month defensive repositioning when asset signals turned flat. The higher win rate in the Monthly Strategy (50.79\% vs 42.57\%) suggests that reduced rebalancing frequency led to fewer tactical adjustments but potentially avoided some whipsaw trades.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{PDFs/Regentfund Monthly Equity Curve.png}
...
The Monthly Strategy achieves a 0.25 Sharpe ratio with 0.74\% annualized return and $-11.99\%$ maximum drawdown while executing approximately one trade per month. The performance gap relative to Strategy 3 (0.45 Sharpe, 1.21\% return) reflects Regentfund's monthly trading constraint rather than framework design limitations. The risk-triggered rebalancing mechanism successfully preserves defensive capabilities, allowing immediate repositioning when Strategy 1 exit conditions are met for any asset.
